...
router.post('/register', async (req: any, res: any) => {
  try {
    ...
    const user: any = await User.create({ email: email.toLowerCase(), password: hashed, name: name || email.split('@')[0], plan: 'free', tokens: 75 });
    const token = jwt.sign({ userId: user._id }, process.env.JWT_SECRET!, { expiresIn: '30d' });
    console.log(`[Roomvera] New user: ${email} — 75 tokens`);
