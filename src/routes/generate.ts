...
async function generateWithStability(imageBase64: string | null, prompt: string): Promise<Buffer> {
  ...
  formData.append('prompt', prompt + ', keep same layout, same windows, same walls, redesign only');
  formData.append('negative_prompt', 'low quality, blurry, ugly, distorted, watermark, people, text, different room layout');
...
router.post('/', requireAuth, requireTokens, async (req: any, res: any) => {
  try {
    ...
    if (!process.env.STABILITY_API_KEY) return res.status(500).json({ error: 'AI service not configured' });
    let inputUrl: string | undefined;
    if (image) {
      const uploaded: any = await cloudinary.uploader.upload(image, { folder: 'roomvera/inputs' });
      inputUrl = uploaded.secure_url;
      console.log(`[Roomvera Generate] Input uploaded: ${inputUrl}`);
    }
    ...
    console.log(`[Roomvera Generate] Created: ${generation._id}`);
    const generateImage = async () => {
      try {
        console.log(`[Roomvera Generate] Calling Stability AI...`);
        const imageBuffer = await generateWithStability(image || null, fullPrompt);
        let uploadOptions: any = { folder: 'roomvera/generated' };
        if (userPlan === 'free') {
          uploadOptions.transformation = [{ overlay: { font_family: 'Arial', font_size: 18, text: 'Roomvera AI' }, gravity: 'south_east', x: 10, y: 10, opacity: 35, color: 'white' }];
        }
        const uploadResult: any = await new Promise((resolve, reject) => {
          cloudinary.uploader.upload_stream(uploadOptions, (err, result) => err ? reject(err) : resolve(result)).end(imageBuffer);
        });
        ...
        console.log(`[Roomvera Generate] Complete!`);
      } catch (err: any) {
        console.error(`[Roomvera Generate] Error:`, err.message);
        await User.findByIdAndUpdate(userId, { $inc: { tokens: 25 } });
    ...
    res.json({ success: true, generationId: generation._id, tokensRemaining: (updatedUser as any)?.tokens, redesignsRemaining: Math.floor(((updatedUser as any)?.tokens || 0) / 25) });
  } catch (e: any) {
    console.error(`[Roomvera Generate] FATAL:`, e.message);
...
router.get('/download/:id', requireAuth, async (req: any, res: any) => {
  try {
    const generation: any = await Generation.findOne({ _id: req.params.id, userId: req.user.id }).lean();
    if (!generation || !generation.imageUrl) return res.status(404).json({ error: 'Not found' });
    res.redirect(generation.imageUrl + '?dl=roomvera-design.png');
  } catch (e: any) { res.status(500).json({ error: e.message }); }
});
